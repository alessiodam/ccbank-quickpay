...
local to_get = {
    "extern:startup.lua:https://raw.githubusercontent.com/tkbstudios/ccbank-quickpay/main/startup.lua",
    "L:logging:logging",
}
local program_name = "Bank Of CC QuickPay"
local pinestore_id = 73 -- Set this to the ID of the pinestore project if you wish to note to pinestore that a download has occurred.
...
local RAW_URL_LIBRARIES = "https://raw.githubusercontent.com/tkbstudios/CC-libs/main/"
...
local function calculate_diffs(version)
    ...
    local function resolve_diff(diff)
        if seen_diffs[diff] then
            print_warning(("Multiple references to diff '%s'. This is probably fine, but if this warning is being spammed you have a loop.")
                :format(diff))
        end
...
if p_dir:match("^rom") then
    error(
        "Attempting to install to the ROM. Please rerun but add arguments for install location (or run the installer script in the folder you wish to install to).",
        0)
end
